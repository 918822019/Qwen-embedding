...
from transformers.image_utils import ChannelDimension, PILImageResampling, get_image_size
try:
...
try:
    ...
    from transformers import ImageProcessingMixin as BaseVideoProcessor
try:
    from transformers.video_utils import VideoMetadata, group_videos_by_shape, reorder_videos
except ImportError:
    # Fallback for older transformers versions
    class VideoMetadata:
        def __init__(self, fps=None, total_num_frames=None, frames_indices=None, **kwargs):
            self.fps = fps
            self.total_num_frames = total_num_frames
            self.frames_indices = frames_indices
    def group_videos_by_shape(videos):
        return {videos[0].shape: torch.stack(videos)}, {videos[0].shape: list(range(len(videos)))}
    def reorder_videos(grouped, index_dict):
        result = []
        for shape, indices in index_dict.items():
            for i, idx in enumerate(indices):
                result.append(grouped[shape][i])
        return result
