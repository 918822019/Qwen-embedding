...
class Qwen3VLVisionBlock(Qwen2_5_VLVisionBlock):
    def __init__(self, config, attn_implementation: str = "sdpa", layer_idx: int = None) -> None:
        ...
        if vis_skip_layer:
            self.layer_skip = vis_skip_layer[layer_idx] if layer_idx is not None else 0
        else:
            self.layer_skip = 0
        ...
        if patch_pos[0] is not None:
            ...
            for idx, (patches_count, comps_count) in enumerate(grid_dims):
                ...
                if disabled.numel() > 0:
                    ...
                    # For each disabled merged token, compute the base patch index
                    ...
                    drop_indices = base_indices.unsqueeze(1) + offsets.unsqueeze(0)
            ...
            # Create new cu_seqlens for selected patches
            selected_cu_seqlens = [0]
            for i in range(1, len(cu_seqlens)):
                start_idx = cu_seqlens[i-1]
                end_idx = cu_seqlens[i]
                selected_count = patch_mask[start_idx:end_idx].sum().item()
                selected_cu_seqlens.append(selected_cu_seqlens[-1] + selected_count)

            selected_cu_seqlens_tensor = torch.tensor(selected_cu_seqlens, device=hidden_states.device, dtype=cu_seqlens.dtype)

            # Process selected hidden states
            selected_hidden_states = selected_hidden_states + self.attn(
                self.norm1(selected_hidden_states),
                cu_seqlens=selected_cu_seqlens_tensor,
                position_embeddings=position_embeddings,
            )
            selected_hidden_states = selected_hidden_states + self.mlp(self.norm2(selected_hidden_states))

            # Put back processed states
            processed_hidden_states[patch_mask] = selected_hidden_states
...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.ModuleList([Qwen3VLVisionBlock(config, layer_idx=i) for i in range(config.depth)])
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, patch_pos=None, select_mask=None, **kwargs) -> torch.Tensor:
        """
        Args:
            ...
            patch_pos (`torch.Tensor`, *optional*):
                The patch position information for UI-guided token selection.
            select_mask (`torch.Tensor`, *optional*):
                The selection mask for UI-guided token selection.
...
class Qwen3VLModel(Qwen2_5_VLModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, patch_pos=None, select_mask=None):
        """
        ...
        image_embeds, deepstack_image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw, patch_pos=patch_pos, select_mask=select_mask)
        split_sizes = (image_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        image_embeds = torch.split(image_embeds, split_sizes)
        return image_embeds, deepstack_image_embeds

    def get_video_features(
        self, pixel_values_videos: torch.FloatTensor, video_grid_thw: Optional[torch.LongTensor] = None, patch_pos=None, select_mask=None
    ):
        """
        ...
        return self.get_image_features(pixel_values_videos, video_grid_thw, patch_pos, select_mask)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        patch_pos (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            The patch position information for UI-guided token selection.
        select_mask (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            The selection mask for UI-guided token selection.
        """
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        ...
        if pixel_values is not None:
            image_embeds, deepstack_image_embeds = self.get_image_features(pixel_values, image_grid_thw, patch_pos, select_mask)
        ...
        if pixel_values_videos is not None:
            video_embeds, deepstack_video_embeds = self.get_video_features(pixel_values_videos, video_grid_thw, patch_pos, select_mask)
