...
try:
    from transformers.processing_utils import MultiModalData, Unpack
except ImportError:
...
def get_select_mask(patch_pos, skip_ratio=0.0, rand=False):
    """Generate selection mask for token selection."""
    import random
    mask = patch_pos >= 0  # True for visual tokens
    if skip_ratio > 0:
        visual_indices = np.where(mask)[0]
        num_to_skip = int(len(visual_indices) * skip_ratio)
        if rand:
            skip_indices = random.sample(list(visual_indices), num_to_skip)
        else:
            skip_indices = visual_indices[-num_to_skip:]
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen3VLProcessorKwargs],
        ...
        if images is not None:
            ...
            patch_assign_len = image_inputs.get("patch_assign_len", None)
        else:
            image_inputs = {}
            image_grid_thw = None
        ...
        if patch_assign_len is not None:
            ...
            while i < len(text_inputs['input_ids'][0]):
                ...
                if text_inputs['input_ids'][0, i] == self.image_token_id:   # <|image_pad|> token
            ...
            # Get uimask_ratio and uimask_rand from kwargs or use defaults
